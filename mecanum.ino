...
void loop() {
  ...
  if(_frequency_controller.delay(millis())) {
    ...
    //DEBUG
    debug_1.x = cos(abs(yaw_est) / 2.0f ) * ((motor_right_rate_est + -motor_left_rate_est + motor_back_right_rate_est + -motor_back_left_rate_est)/80);
    debug_1.y = sign(yaw_est) * sin(abs(yaw_est) / 2.0f ) * ((motor_right_rate_est + -motor_left_rate_est + motor_back_right_rate_est + -motor_back_left_rate_est)/80);
    debug_1.z = yaw_est;
  ...
  if(_frequency_odometry.delay(millis())) {
   ...
   // standing still  
    ...
    if ((motor_right_rate_est > -0.1 && motor_left_rate_est < 0.1) and (motor_back_right_rate_est < 0.1 && motor_back_left_rate_est > -0.1)){
     ...
     //  linear_velocity_est_x = cos(abs(yaw_est) / 2.0f ) * ((motor_right_rate_est + -motor_left_rate_est + motor_back_right_rate_est + -motor_back_left_rate_est)/80);
     //  linear_velocity_est_y = sign(yaw_est) * sin(abs(yaw_est) / 2.0f ) * ((motor_right_rate_est + -motor_left_rate_est + motor_back_right_rate_est + -motor_back_left_rate_est)/80); 
     //  angular_velocity_est_z = (WHEEL_RADIUS / BASE_LENGTH) * (motor_right_rate_est + motor_left_rate_est + motor_back_right_rate_est + motor_back_left_rate_est); 
     
     linear_velocity_est_x = (motor_right_rate_est - motor_left_rate_est - motor_back_right_rate_est + motor_back_left_rate_est)/4;
     linear_velocity_est_y = (motor_right_rate_est + motor_left_rate_est + motor_back_right_rate_est + motor_back_left_rate_est)/4;
     angular_velocity_est_z = (-motor_right_rate_est + motor_left_rate_est - motor_back_right_rate_est + motor_back_left_rate_est)/4/BASE_LENGTH;  

    }
    
    // compute translation and rotation
    // Quaternion data
    yaw_est += angular_velocity_est_z * dt;
